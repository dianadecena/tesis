...
def obtenerArchivos(ruta):
	with os.scandir(ruta) as ficheros:
		for fichero in ficheros:
			...
			mostrarErrores(path)
			if tipo_indicador[8] == "Ataques Bloqueados":
				guardarDatosVar(3, 'seguridad', path, 'id_ataque', 'seg_ataque')
			elif tipo_indicador[8] == "Incidentes":
				guardarDatosVar(3, 'seguridad', path, 'id_incidente', 'inciden_seg')
			elif tipo_indicador[8] == "Cambios":
				guardarDatosVar(3, 'seguridad', path, 'id_cambios', 'cambios_seg')
			else:
...
nombres_indicadores = ["Ataques Bloqueados", "Incidentes", "Cambios", "Satisfaccion Usuarios"]

for nombre in nombres_indicadores:
	ruta = "/Users/Usuario/Documents/Tesis Diana 2021/Codigo/Datos de Prueba/Seguridad/"+nombre+"/"
