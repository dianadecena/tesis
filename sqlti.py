...
def guardarDatosVar(num_area, area, archivo, id_var, tabla_var):
        #conectar con la base de datos 
        connection = conectarBD()
        df = pd.read_excel(archivo, engine='openpyxl')
        df_var = df[[id_var]]
        df_area = selectArea(num_area, area, connection, df)
        guardarTablaVar(connection, df_area, df_var, tabla_var)
        ultimo_ind = selectUltimoInd(connection)
        guardarInd(connection, df)
        selectIndicador(num_area, connection, ultimo_ind, df_area)
...
def guardarTablaVar(connection, df_des, df_var, tabla_var):
        try:
            nuevo_df = pd.concat([df_des, df_var], axis=1)
            nuevo_df.to_sql(tabla_var, con=connection[1], if_exists='append', index=False)
        except Exception as e: 
            alertaError()
